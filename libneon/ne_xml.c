...
static const struct ne_xml_elm root_element = 
{ "@<root>@", NE_ELM_root, 0 };
...
static int find_handler(ne_xml_parser *p, struct ne_xml_state *state) 
{
	...
	for (n = 0; (cur->elements[n].nspace != NULL ||
		...
		switch (cur->validate_cb(cur->userdata, 
					 state->parent->elm->id, 
					 cur->elements[n].id)) {
		case NE_XML_VALID:
			  ...
			  const ne_xml_char **atts) 
{
	...
	if (atts != NULL) {
		...
		ne_buffer_concat(p->buffer, " ", atts[n], 
				 "=\"", atts[n+1], "\"", NULL);
	    }
	}
...
static void char_data(void *userdata, const ne_xml_char *data, int len) 
{
    ...
    if (p->current->mixed) {
	p->current->handler->cdata_cb(p->current->handler->userdata,
				      p->current->elm, data, len);
	return;
    }
    ...
    if ((p->current->elm->flags & NE_XML_STRIPWS) &&
	ne_buffer_size(p->buffer) == 0) {
	int wslen = 0;
...
/* Find a namespace definition for 'prefix' in given parser state,
 * where length of prefix is 'pfxlen'.  Returns the URI or NULL. */
static const char *
resolve_nspace(const struct ne_xml_state *state, 
	       const char *prefix, size_t pfxlen)
{
    const struct ne_xml_state *s;

    /* Search up the tree. */
    for (s = state; s != NULL; s = s->parent) {
	const struct ne_xml_nspace *ns;
			 ...
			 const ne_xml_char *name, const ne_xml_char **atts)
{
	...
	for (attn = 0; atts[attn]!=NULL; attn+=2) {
		...
		NE_DEBUG(NE_DBG_XMLPARSE, "New default namespace: %s\n", 
			 state->default_ns);
		/* FIXME: if attribute value is empty, then this means
		...
		if (atts[attn+1][0] == '\0') {
		    ...
		    ne_snprintf(p->error, ERR_SIZE, 
				"Invalid namespace declaration for '%s' "
				"in '%s' at line %d.",
				atts[attn], name, ne_xml_currentline(p));
		    return -1;
		}
		/* New namespace scope */
	...
	state->elm_real.nspace = resolve_nspace(state, name, pnt-name);
