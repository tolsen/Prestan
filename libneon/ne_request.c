...
static int aborted(ne_request *req, const char *doing, ssize_t code)
{
    ...
    default:
	if (sess->socket != NULL) {
	    const char *err = ne_sock_error(sess->socket);
...
static void add_fixed_headers(ne_request *req) 
{
    ...
    //ne_buffer_zappend(req->headers, "Cache-Control: no-store" EOL);
}
			       ...
			       char *buffer, size_t *buflen) 
{
    ...
    *buflen = (size_t)readlen;
	...
	if (resp->chunk_left == 0) {
		...
		return aborted(req, _("Error reading chunked response body"),
			       readlen);
	    }
	}
...
int ne_begin_request(ne_request *req)
{
    ...
    if (st->major_version > 1 || 
	(st->major_version == 1 && st->minor_version > 0))
	/* Remember that server claims HTTP/1.1 compliance. */
	req->session->is_http11 = 1;
...
#endif
	sess->socket = ne_sock_connect(host->current, host->port);
    } while (sess->socket == NULL && /* try the next address... */
	     (host->current = ne_addr_next(host->address)) != NULL);

    if (sess->socket == NULL) {
	aborted(req, err, NE_SOCK_ERROR);
	return NE_CONNECT;
    }
