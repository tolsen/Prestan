 ...
 * given handler, as returned by the ne_props_alloc_complex callback
...
/* If adding handlers to the parser returned by get_parser, the first
 * element ID which should be used is this: */
#define NE_ELM_PROPS_UNUSED (NE_ELM_207_UNUSED + 100)
 ...
 * response, the 'allocator' callback is called to allocate this
 * private.  When the private structure is no longer needed, the
 * 'freer' callback is called to deallocate it, if non-NULL.
 *
 * Whilst in the XML element callbacks you will have registered to handle
 ...
 * */
typedef void *(*ne_props_alloc_complex)(void *userdata,
                                        const char *href);

/* 'complex' is the pointer returned by the alloc_complex call. */
typedef void (*ne_props_free_complex)(void *userdata, void *complex);

/* Install the allocation/deallocation callbacks as above: allocfn
 * must be non-NULL, freefn may be NULL. */
void ne_propfind_set_private(ne_propfind_handler *handler,
			     ne_props_alloc_complex allocfn,
                             ne_props_free_complex freefn,
			     void *userdata);
